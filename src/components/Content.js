			...
			<div className=" h-screen flex-grow overflow-x-hidden overflow-auto flex flex-wrap content-start p-2">
				...
				<div className="w-full border-gray-200 dark:border-gray-800 border-b overflow-y-auto lg:block hidden p-5">
					...
					<div className="flex flex-row space-x-4 mt-3">
						...
						<div key={id} className={toggle === id ? "rounded-lg bg-card p-3 w-full flex flex-col rounded-md dark:bg-gray-800 shadow-lg relative ring-2 ring-blue-500 focus:outline-none" : "rounded-lg bg-card p-3 w-full flex flex-col rounded-md dark:bg-gray-800 shadow"} onClick={() => handleChangeToggle(id)}>
							<div style={{cursor: 'pointer'}} className="flex xl:flex-row flex-col items-center font-medium text-white-900 pb-2 mb-2 xl:border-b border-gray-200 border-opacity-75 dark:border-gray-700 w-full">
							<IconButton icon="https://svgshare.com/i/gj6.svg" className="w-8 h-8 mr-5" />
								{name}
							</div>
							<div className="flex items-center w-full">
								<div className="text-xs py-1 px-2 leading-none dark:bg-gray-900 bg-yellow-100 text-yellow-600 rounded-md">Address</div>
								<div className="ml-auto text-xs text-gray-400">{address}</div>
							</div>
						</div>
						))}
					</div>
